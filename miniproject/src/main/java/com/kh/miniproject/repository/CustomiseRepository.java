...
public class CustomiseRepository {
    ...
    private final String GET_CAR_INFO_FOR_CUSTOM = "SELECT * FROM VM_CAR_DETAIL";
    private final String GET_COLOR_INFO_FOR_CUSTOM = "SELECT * FROM VM_CAR_COLOR";
    private final String GET_FEATURE_INFO_FOR_CUSTOM = "SELECT * FROM VM_CAR_FEATURE";

    public List<CarVo> getCarInfo() {
        return jdbcTemplate.query(GET_CAR_INFO_FOR_CUSTOM, new CarRowMapper());
    }

    public List<ColorVo> getColorInfo() {
        return jdbcTemplate.query(GET_COLOR_INFO_FOR_CUSTOM, new ColorRowMapper());
    }

    public List<FeatureVo> getFeatureInfo() {
        return jdbcTemplate.query(GET_FEATURE_INFO_FOR_CUSTOM, new FeatureRowMapper());
    }
        ...
        public CarVo mapRow(ResultSet rs, int rowNum) throws SQLException {
            return new CarVo(rs.getInt("CAR_NO"), rs.getString("CAR_NAME"), rs.getString("CLASSIFICATION"),
                   null, rs.getString("ENGINE_TYPE"), rs.getDouble("DISPLACEMENT"), rs.getInt("HORSEPOWER"),
                    ...
                    rs.getString("CAR_AROUND_URL"), rs.getString("CAR_3D_URL"), rs.getString("CAR_DESC"), rs.getString("CAR_SUMMARY"),
                    rs.getString("MANUFACTURER"), rs.getString("MANUFACTURER_URL"));
        }
    }

    // 차량 색상정보, 견적시에 페이지에서 보여줄 객체 맵핑
